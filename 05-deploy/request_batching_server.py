...
# we only run 1 inference run at any time
# (one could schedule between several runners if desired)
MAX_QUEUE_SIZE = 3  # we accept a backlog of MAX_QUEUE_SIZE before handing out "Too busy" errors  # noqa: E501
MAX_BATCH_SIZE = 2  # we put at most MAX_BATCH_SIZE things in a single batch  # noqa: E501
MAX_WAIT = 1        # we wait at most MAX_WAIT seconds before running for more inputs to arrive in batching  # noqa: E501
...
class ModelRunner:
    ...
    def run_model(self, batch):  # runs in another thread
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        logger.error(
            'Call as {} <traced_zebra_model_path>'.format(
                sys.argv[0]
            )
        )
        sys.exit(1)
